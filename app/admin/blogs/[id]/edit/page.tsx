...
export default function EditBlogPage() {
    ...
    (async () => {
      try {
        ...
        if (!r.ok) {
          setError(
            data?.message ||
              (typeof data === "string" ? data : `Failed to load blog #${id}`)
          );
          setLoading(false);
          return;
        }

        const b = normalizeBlog(data);
        if (!b) {
          setError("Unable to parse blog payload.");
          setLoading(false);
          return;
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      // Use PUT to match your current backend
      const r = await apiFetch(`/api/blogs/${encodeURIComponent(String(id))}`, {
        method: "PUT",
        body: JSON.stringify(payload),
      });
      ...
      if (!r.ok) {
        const msg = (await r.text()) || "Update failed";
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4">
      <div className="w-full max-w-2xl">
        ...
        {error && (
          <p
            className="mb-4 rounded border border-red-200 bg-red-50 px-3 py-2 text-red-700 text-center break-words"
            role="alert"
          >
            {error}
          </p>
